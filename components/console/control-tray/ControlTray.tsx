...
function ControlTray({ children }: ControlTrayProps) {
  ...
  useEffect(() => {
    micSuppressedRef.current = isMicSuppressed;

    if (connected && !muted) {
      audioRecorder.setInputEnabled(!isMicSuppressed);
    }
    if (isMicSuppressed) {
      setMicVolume(0);
    }
  }, [audioRecorder, connected, muted, isMicSuppressed]);
  ...
  useEffect(() => {
    ...
    if (connected && !muted && audioRecorder) {
      ...
      audioRecorder
        .start()
        .then(() => {
          audioRecorder.setInputEnabled(!micSuppressedRef.current);
        })
        .catch((error: unknown) => {
  ...
  const isMicActive = connected && !muted && !isMicSuppressed;
  const streamingStateLabel = connected
    ? isMicSuppressed
      ? 'Speaking'
      : 'Listening'
    : 'Streaming';
    ...
    <section className="control-tray">
      ...
      <div className="control-tray-buttons">
        ...
        <div className={cn('connection-container', { connected })}>
          ...
          <span
            className={cn('text-indicator', {
              listening: connected && !isMicSuppressed,
              speaking: connected && isMicSuppressed,
            })}
            aria-live="polite"
          >
            {streamingStateLabel}
